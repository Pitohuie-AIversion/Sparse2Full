...
class DarcyFlowDataset(Dataset):
        ...
        **kwargs
        ...
        with h5py.File(self.data_path, 'r') as f:
            ...
            if "tensor" in f:
                ...
                raise ValueError(f"Invalid DarcyFlow file: 'tensor' key not found in {data_path}")
                
        # Determine indices for this split
        self.indices = self._get_split_indices(self.n_samples, train_ratio, val_ratio, test_ratio)
        ...
        if self.splits_dir:
            ...
            if split_file.exists():
                ...
                try:
                    ...
                    print(f"Warning: Failed to read split file {split_file}: {e}. Falling back to ratio.")
    ...
    def _compute_norm_stats(self):
        ...
        if stat_file and stat_file.exists():
            ...
            for k in self.keys:
                self.stats_mean[k] = torch.tensor(data[f"{k}_mean"], dtype=torch.float32)
        ...
        train_indices = self._get_split_indices(self.n_samples, 0.8, 0.1, 0.1) # Default ratios?
        ...
        print(f"Computing normalization stats for {self.split} (Warning: Should use train stats for all)")
        ...
        with h5py.File(self.data_path, 'r') as f:
            for k in self.keys:
                ...
                self.stats_mean[k] = torch.tensor(data_tensor.mean(), dtype=torch.float32)
...
class DarcyFlowDataModule(pl.LightningDataModule):
    ...
    def __init__(self, config: DictConfig):
        ...
        data_cfg = config.get('data', {}) if isinstance(config, dict) else getattr(config, 'data', None)
        if data_cfg is None:
        ...
        if obs_cfg:
            ...
            if mode == 'SR':
                 ...
                 if "blur_kernel_size" in self.h_params and "kernel_size" not in self.h_params:
        ...
        self.batch_size = dl_cfg.get('batch_size', config.training.get('batch_size', 32))
    ...
    def setup(self, stage: Optional[str] = None):
        if stage == "fit" or stage is None:
            ...
            # Share stats
            if self.normalize:
                self.val_dataset.stats_mean = self.train_dataset.stats_mean.copy() if hasattr(self.train_dataset.stats_mean, 'copy') else self.train_dataset.stats_mean
                self.val_dataset.stats_std = self.train_dataset.stats_std.copy() if hasattr(self.train_dataset.stats_std, 'copy') else self.train_dataset.stats_std
