...
class SequentialDCConsistency(nn.Module):
    ...
    def apply_h_operator(self, x: torch.Tensor, params: Optional[Dict] = None) -> torch.Tensor:
        """
        应用H算子到输入数据
    ...
    def verify_spatial_consistency(self, spatial_pred: torch.Tensor, observation: torch.Tensor) -> Dict[str, float]:
        """
        验证空间预测的一致性
    ...
    def verify_temporal_consistency(self, temporal_pred: torch.Tensor, observation: torch.Tensor) -> Dict[str, float]:
        """
        验证时间预测的一致性
    ...
    def compute_dc_loss(self, pred: torch.Tensor, observation: torch.Tensor) -> torch.Tensor:
        """
        计算数据一致性损失
...
class SequentialConsistencyChecker:
    ...
    def check_stage_consistency(self, spatial_output, temporal_output, observation) -> Dict[str, Dict[str, float]]:
        """
        检查两阶段的一致性
    ...
    def _check_stage_transition(self, spatial_pred: torch.Tensor, temporal_pred: torch.Tensor) -> Dict[str, float]:
        """
        检查两阶段间的转换一致性
    ...
    def compute_total_consistency_loss(self, spatial_pred: torch.Tensor, temporal_pred: torch.Tensor, observation: torch.Tensor) -> torch.Tensor:
        """
        计算总的一致性损失
        ...
        # 时间预测DC损失
        temporal_dc_loss = self.dc_consistency.compute_dc_loss(temporal_pred, observation)
        
        # 两阶段一致性损失
        stage_consistency_loss = torch.nn.functional.mse_loss(temporal_pred, spatial_pred)
