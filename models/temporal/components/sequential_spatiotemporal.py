...
class SpatialFeatureExtractor(nn.Module):
    ...
    def __init__(self, in_channels: int, feature_dim: int, img_size: Tuple[int, int], backbone_type: str = "simple", backbone_config: Dict = None):
        super().__init__()
            ...
            if pred.dim() == 5 and target.dim() == 5:
                ...
                if Tp != Tt:
                    if Tp > Tt:
                        ...
                        pad = Tt - Tp
                        pred = torch.cat([pred, pred[:, -1:].repeat(1, pad, 1, 1, 1)], dim=1)
...
class SpatialPredictionModule(nn.Module):
    ...
    def forward(self, x: torch.Tensor, target: Optional[torch.Tensor] = None) -> SpatialPredictionOutput:
        """
        空间预测前向传播
        Args:
        ...
        if self.feature_extractor is None:
            ...
            spatial_features = torch.zeros(B, T_in, 1, H, W, device=x.device, dtype=x.dtype)
...
class TemporalFeatureExtractor(nn.Module):
    ...
    def __init__(self, input_dim: int, temporal_dim: int, num_heads: int = 8, num_layers: int = 4, dropout: float = 0.1, use_positional_encoding: bool = False):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.input_proj.in_features != x.shape[-1]:
            raise RuntimeError(f"TemporalFeatureExtractor input mismatch: Model expects {self.input_proj.in_features}, got {x.shape[-1]}")
...
class TemporalPredictionModule(nn.Module):
    ...
    def __init__(self, config: Dict):
        ...
        if reduce_spatial == 'avg_pool' and reduce_size is not None:
            input_dim = max(1, (spatial_feature_dim + out_channels) * reduce_size[0] * reduce_size[1])
        else:
            input_dim = max(1, (spatial_feature_dim + out_channels) * img_size[0] * img_size[1])
        
        if self.backend == 'tcn':
            from models.temporal.components.temporal_encoder import create_temporal_encoder
            self.temporal_encoder = create_temporal_encoder(
                ...
                config={
                    ...
                    'use_positional_encoding': bool(config.get('use_positional_encoding', True)),
                }
            )
            ...
            use_spatial_features = bool(config.get('use_spatial_features', False))
            effective_feature_dim = spatial_feature_dim if use_spatial_features else 0
            ...
            self.feature_extractor = TemporalFeatureExtractor(
                ...
                use_positional_encoding=bool(config.get('use_positional_encoding', False))
            )
            self.prediction_head = TemporalPredictionHead(
        ...
        if isinstance(reduce_spatial, str) and reduce_spatial.lower() in ('avgpool', 'avg_pool', 'avg'):
            self.reduce_spatial = 'avg_pool'
        else:
            self.reduce_spatial = reduce_spatial
        self.reduce_size = reduce_size
        
    def forward(self, spatial_results: SpatialPredictionOutput, target: Optional[torch.Tensor] = None) -> TemporalPredictionOutput:
        """
        时间预测前向传播
        Args:
        ...
        use_spatial_features = bool(self.config.get('use_spatial_features', False))
        ...
        if self.reduce_spatial == 'avg_pool' and self.reduce_size is not None:
            ...
            if spatial_pred.dim() == 5:
                ...
                spatial_pred = sp.reshape(b, t, c, self.reduce_size[0], self.reduce_size[1])
            else:
        ...
        if T_pred != T_out:
            if T_pred == 1:
                ...
                pad = T_out - T_pred
                spatial_pred_flat = torch.cat([spatial_pred_flat, last.expand(B, pad, -1)], dim=1)
        ...
        if spatial_features is not None:
            ...
            if self.reduce_spatial == 'avg_pool' and self.reduce_size is not None:
                ...
                if spatial_features.dim() == 5:
                    ...
                    spatial_features = sf.reshape(b, t, c, self.reduce_size[0], self.reduce_size[1])
                elif spatial_features.dim() == 4:
                    spatial_features = F.adaptive_avg_pool2d(spatial_features, self.reduce_size)
            
            # 展平与规范化为 [B, T_feat, F]
            if spatial_features.dim() == 5:
                spatial_features_flat = spatial_features.reshape(B, spatial_features.shape[1], -1)
            elif spatial_features.dim() == 4:
            ...
            if T_feat != T_out:
                if T_feat == 1:
                    ...
                    pad = T_out - T_feat
                    spatial_features_flat = torch.cat([spatial_features_flat, last.expand(B, pad, -1)], dim=1)
        else:
        ...
        if self.backend == 'tcn':
            ...
            if self.prediction_head is not None:
                ...
                if last_linear is None or last_linear.out_features != out_dim:
                     raise RuntimeError(f"TCN Output Head mismatch: Expected out_features={out_dim}, got {last_linear.out_features if last_linear else 'None'}")
            ...
            B, T, _ = temporal_input.shape
            temporal_features = self.physics_model.encode_temporal_features(temporal_input)
            ...
            h_red, w_red = self.physics_model.img_size if hasattr(self.physics_model, 'img_size') else (256, 256)
                ...
                if self.prediction_head is not None:
                    for m in self.prediction_head.temporal_to_spatial:
                        if isinstance(m, nn.Linear):
                            last_linear = m
                    if last_linear is None or last_linear.out_features != out_dim:
                         raise RuntimeError(f"Prediction Head mismatch: Expected out_features={out_dim}, got {last_linear.out_features if last_linear else 'None'}")
...
class SequentialSpatiotemporalModel(nn.Module):
                ...
                if mixed_spatial_pred.dim() == 5 and target.dim() == 5:
                    ...
                    if mixed_spatial_pred.shape[1] != Tt:
                        if mixed_spatial_pred.shape[1] > Tt:
                            ...
                            mixed_spatial_pred = torch.cat([mixed_spatial_pred, mixed_spatial_pred[:, -1:].repeat(1, pad_frames, 1, 1, 1)], dim=1)
                    # 生成时间步级别的采样掩码
                    mask = (torch.rand(Bt, Tt, device=mixed_spatial_pred.device) < float(self.teacher_prob)).float()
    ...
    def set_epoch(self, epoch: int, decay: Optional[float] = None):
        try:
    ...
    def temporal_forward(self, spatial_results: SpatialPredictionOutput, target: Optional[torch.Tensor] = None) -> TemporalPredictionOutput:
        """仅执行时间预测"""
        return self.temporal_module(spatial_results, target)
    
    def rollout_inference(self, x: torch.Tensor, T_out: int, step_by_step: bool = True, preserve_grad: bool = False) -> torch.Tensor:
        """
        ...
        if step_by_step:
            ...
            with torch.set_grad_enabled(preserve_grad):
            ...
            if not preserve_grad and was_training:
                self.train()
