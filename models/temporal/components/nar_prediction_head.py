        ...
        if strategy == "normal":
            ...
            for t in range(self.T_out):
                ...
                for d in range(self.hidden_dim):
                    if d % 2 == 0:
                        self.time_queries.data[t, d] = math.sin(t / (10000 ** (d / self.hidden_dim)))
                    else:
                        self.time_queries.data[t, d] = math.cos(t / (10000 ** ((d-1) / self.hidden_dim)))
        else:
...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(
        ...
        # 重塑为多头格式
        Q = Q.view(B, T_out, self.num_heads, self.head_dim).transpose(1, 2)  # [B, num_heads, T_out, head_dim]
        K = K.view(B, T_in, self.num_heads, self.head_dim).transpose(1, 2)   # [B, num_heads, T_in, head_dim]
        V = V.view(B, T_in, self.num_heads, self.head_dim).transpose(1, 2)   # [B, num_heads, T_in, head_dim]
        
        # 计算注意力分数
        attn_scores = torch.matmul(Q, K.transpose(-2, -1)) * self.scale  # [B, num_heads, T_out, T_in]
        ...
        # 重塑回原始格式
        attn_output = attn_output.transpose(1, 2).contiguous().view(B, T_out, self.hidden_dim)
...
class CrossAttnTimeQueryHead(nn.Module):
    ...
    def __init__(
        ...
        # 位置编码（可选）
        if use_pos_encoding:
            self.pos_encoding = nn.Parameter(torch.randn(1000, hidden_dim) * 0.02)  # 支持最多1000个时间步
    ...
    def get_attention_weights(
        ...
        if self.use_pos_encoding:
            pos_enc = self.pos_encoding[:x.size(1)].unsqueeze(0).expand(x.size(0), -1, -1)
            x = x + pos_enc
        ...
        for i in range(target_layer + 1):
            if i == target_layer:
                ...
                Q = Q.view(B, T_out, attn_layer.num_heads, attn_layer.head_dim).transpose(1, 2)
                K = K.view(B, T_in, attn_layer.num_heads, attn_layer.head_dim).transpose(1, 2)
