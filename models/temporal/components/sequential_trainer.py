...
class TemporalTrainer:
    ...
    def validate(self, spatial_outputs, val_loader: DataLoader, dc_consistency, spatial_module=None) -> Dict[str, float]:
        """验证时间模型"""
        self.model.eval()
        total_metrics = {}
        num_batches = 0
        ...
        with torch.no_grad():
            for i, batch in enumerate(val_loader):
                ...
                # 获取数据并移到设备
                batch = {k: v.to(spatial_module.device if spatial_module else self.model.device) if isinstance(v, torch.Tensor) else v for k, v in batch.items()}
    ...
    def train(self, train_loader: DataLoader, val_loader: Optional[DataLoader] = None) -> Dict[str, Any]:
        ...
        for epoch in range(self.temporal_pretrain_epochs):
            ...
            if val_loader is not None:
                ...
                val_metrics = self.temporal_trainer.validate(val_spatial_outputs, val_loader, self.consistency_checker.dc_consistency, spatial_module=self.model.spatial_module)
        ...
        for batch in train_loader:
            ...
            if T_in > 1:
                ...
                spatial_batch = {
                    ...
                    'target': batch['target'] if 'target' in batch else None, # target通常是序列，这里可能不匹配
                    'observation': batch.get('observation')
                }
        ...
        for i, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                if self.model.spatial_module is not None:
                    ...
                    spatial_output.spatial_features = torch.zeros(B, T_in, 1, H, W, device=self.device)
    ...
    def _train_joint_epoch(self, train_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        for batch in train_loader:
            ...
            if self.spatial_trainer:
                ...
                spatial_output = type('SpatialOutput', (), {})()
                spatial_output.spatial_pred = input_data[:, -1:].clone() # Use last input frame
                spatial_output.spatial_features = torch.zeros(B, T_in, 1, H, W, device=self.device)
                spatial_output.spatial_metrics = {}
            ...
            if self.spatial_trainer:
                spatial_pred = spatial_output.spatial_pred
                spatial_recon_loss = self.spatial_trainer.reconstruction_loss(spatial_pred, target_data)
            ...
            if observation is not None:
                try:
                    ...
                    # 应用退化算子 H
                    if hasattr(self.consistency_checker.dc_consistency, 'degradation_op'):
                        pred_obs_flat = self.consistency_checker.dc_consistency.degradation_op(pred_flat)
                        # 恢复时间维
                        pred_obs_seq = pred_obs_flat.reshape(B, T, -1, *pred_obs_flat.shape[2:])
            ...
            if self.spatial_trainer:
                spatial_pred = spatial_output.spatial_pred
                consistency_loss = self.temporal_trainer.reconstruction_loss(final_pred, spatial_pred)
                w_consistency = self.temporal_trainer.config.get('consistency_loss_weight', 0.5)
            ...
            # 收集指标
            batch_metrics = {
                'spatial_loss': spatial_recon_loss.item() if isinstance(spatial_recon_loss, torch.Tensor) else spatial_recon_loss,
                'temporal_loss': temporal_recon_loss.item() if isinstance(temporal_recon_loss, torch.Tensor) else temporal_recon_loss,
    ...
    def _validate_joint(self, val_loader: DataLoader) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                final_pred = temporal_output.final_pred
                temporal_recon_loss = self.temporal_trainer.reconstruction_loss(final_pred, target_data)
                ...
                dc_loss = 0.0
                if observation is not None:
                    dc_loss = self.consistency_checker.dc_consistency.compute_dc_loss(final_pred, observation)
                
                w_spatial = self.spatial_trainer.config.get('spatial_loss_weight', 1.0) if self.spatial_trainer else 0.0
                w_temporal = self.temporal_trainer.config.get('temporal_loss_weight', 1.0)
                w_consistency = self.temporal_trainer.config.get('consistency_loss_weight', 0.5)
