...
def create_model(model_name: str, **kwargs) -> nn.Module:
    ...
    # 时序Swin模型
    elif lower in {"swintemporal", "swin_temporal", "swintemporalnar", "swin_temporal_nar"}:
        # 懒加载：仅在选择到对应模型时导入，避免未使用模块的顶层导入告警
        from models.temporal.wrappers.swin_temporal import SwinTemporal, SwinTemporalNAR
        return SwinTemporal(**kwargs) if lower in {"swintemporal", "swin_temporal"} else SwinTemporalNAR(**kwargs)
