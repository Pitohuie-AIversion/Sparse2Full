...
class BaseTemporalModel(nn.Module, ABC):
    ...
    def __init__(
        ...
        if mode not in ['ar', 'nar', 'both']:
            raise ValueError(f"Unsupported mode: {mode}. Must be 'ar', 'nar', or 'both'")
        
        logger.info(f"BaseTemporalModel: {in_channels}->{out_channels}, T_in={T_in}, T_out={T_out}, mode={mode}")
    ...
    def get_memory_usage(self, batch_size: int = 1, T_out: Optional[int] = None) -> Dict[str, float]:
        """估算显存使用量（MB）"""
        if T_out is None:
            T_out = self.T_out
        ...
        for name, module in self.named_modules():
            if isinstance(module, (nn.Conv2d, nn.Linear)):
                ...
                if hasattr(module, 'out_features'):
                    ...
                    activation_memory += module.out_channels * self.img_size ** 2 * 4 * 2 / (1024 ** 2)
    ...
    def validate_input(self, x: torch.Tensor) -> None:
        ...
        if H != self.img_size or W != self.img_size:
            logger.warning(f"Input spatial size mismatch: expected {self.img_size}x{self.img_size}, got {H}x{W}")
...
class TemporalConsistencyMixin:
    ...
    def check_temporal_consistency(
        ...
        """检查时序一致性"""
        if pred_seq.dim() != 5:
            raise ValueError(f"Predicted sequence must be 5D [B, T, C, H, W], got {pred_seq.dim()}D")
        ...
        if gt_seq is not None:
            if gt_seq.shape != pred_seq.shape:
                logger.warning(f"Shape mismatch: pred {pred_seq.shape} vs gt {gt_seq.shape}")
            else:
                # 计算时序一致性误差
...
def validate_temporal_model_consistency(
    ...
    try:
        ...
        # 检查输出形状一致性
        consistency_results['shape_consistency'] = (spatial_output.shape == temporal_output.shape)
        ...
        range_ratio = abs(spatial_range.item() - temporal_range.item()) / max(spatial_range.item(), 1e-8)
        ...
        mean_diff_ratio = abs(spatial_mean - temporal_mean) / max(abs(spatial_mean), 1e-8)
...
class TestTemporalModel(BaseTemporalModel, TemporalConsistencyMixin):
    ...
    def forward(
        ...
        # 复制到多个时间步
        output = output.unsqueeze(1).repeat(1, T_out, 1, 1, 1)  # [B, T_out, C_out, H, W]
