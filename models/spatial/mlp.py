...
class MLPModel(BaseModel):
    ...
    def _build_mlp(
        ...
        if activation in ('relu', 'ReLU'):
            ...
            act_fn = nn.GELU
        elif activation in ('swish', 'silu', 'SiLU'):
            act_fn = nn.SiLU
        else:
    ...
    def _forward_patch_mode(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        # 适配输入维度到MLP首层期望in_features
        try:
            expected_in = getattr(self.mlp[0], 'in_features', mlp_input.shape[-1])
        except Exception:
            expected_in = mlp_input.shape[-1]
        if mlp_input.shape[-1] != expected_in:
            if not hasattr(self, 'input_proj') or getattr(self.input_proj, 'in_features', None) != mlp_input.shape[-1] or getattr(self.input_proj, 'out_features', None) != expected_in:
                self.input_proj = nn.Linear(mlp_input.shape[-1], expected_in).to(mlp_input.device)
            mlp_input = self.input_proj(mlp_input)
...
class PatchProcessor(nn.Module):
    ...
    def __init__(self, patch_size: int, overlap: float, img_size: int):
        ...
        # 计算步长
        self.stride = max(int(patch_size * (1 - overlap)), 1)
        ...
        for i in range(n_h):
            for j in range(n_w):
                ...
                if ph != self.patch_size or pw != self.patch_size:
                    pad_bottom = self.patch_size - ph
                    pad_right = self.patch_size - pw
