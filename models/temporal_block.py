...
class TemporalTransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if T <= self.max_seq_len:
            ...
            logger.warning(f"Sequence length {T} exceeds max_seq_len {self.max_seq_len}")
            # 截断或循环使用位置编码
            pos_enc = self.pos_encoding[:, :self.max_seq_len, :].repeat(1, (T // self.max_seq_len) + 1, 1)
            x = x + pos_enc[:, :T, :]
...
class FiLMTemporalBlock(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f"FiLMTemporalBlock: {c_in}->{self.c_out}, hidden={self.hidden_dim}")
    ...
    **kwargs
    ...
    if temporal_type == 'conv1d':
        ...
        transformer_kwargs = {k: v for k, v in kwargs.items() 
                            if k in ['nhead', 'num_layers', 'dim_feedforward', 'dropout', 'causal', 'max_seq_len']}
        ...
        if d_model % nhead != 0:
            ...
            logger.warning(f"Adjusted nhead from {kwargs.get('nhead', 8)} to {nhead} to match d_model={d_model}")
