...
# Port 29501
nohup torchrun --nproc_per_node=2 --master_port=29501 tools/training/train_real_data_ar.py \
...
if ps -p $PID_UNET > /dev/null; then
   ...
   echo "❌ UNet process $PID_UNET failed to start. Check runs_3loss_ablation_unet/nohup_A2_RecDC.log"
   tail -n 20 runs_3loss_ablation_unet/nohup_A2_RecDC.log
fi
...
# Port 29502
nohup torchrun --nproc_per_node=2 --master_port=29502 tools/training/train_real_data_ar.py \
...
if ps -p $PID_EDSR > /dev/null; then
   ...
   echo "❌ EDSR process $PID_EDSR failed to start. Check runs_3loss_ablation/nohup_A2_RecDC_EDSR.log"
   tail -n 20 runs_3loss_ablation/nohup_A2_RecDC_EDSR.log
fi
