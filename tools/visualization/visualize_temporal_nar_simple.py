...
def smooth_curve(data, sigma=1.0):
    """使用高斯滤波平滑曲线"""
    try:
...
🎯 收敛分析
• 收敛轮次: {stats['convergence_epoch'] if stats['convergence_epoch'] else '未检测到明显收敛点'}
...
• 建议: {'模型训练良好，可以使用' if stats['best_val_loss'] and stats['best_val_loss'] < 1.0 else '建议继续训练或调整超参数'}
"""
