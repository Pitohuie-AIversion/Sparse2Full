...
def run_check_from_h5(h5_path: str, tolerance: float = 1e-8, params_override: Dict[str, Any] | None = None) -> Dict[str, Any]:
    p = Path(h5_path)
    ...
    with h5py.File(p, 'r') as f:
        ...
        if 'gt' in f and 'obs' in f:
            ...
            assert data_ds is not None, "未在HDF5中找到 'data' 数据集，也不包含 'gt'/'obs'"
            gt = _to_bchw(data_ds)
...
def main():
    ...
    if args.config:
        ...
        params_override = {
            ...
            'downsample_interpolation': getattr(obs_cfg, 'downsample_interpolation', 'area')
        }
        if not h5_path:
            # 默认从配置的数据路径读取
