...
class RealDataDataset(Dataset):
    ...
    def __init__(self, data_path: str, T_in: int = 1, T_out: int = 5, 
    ...
    def _compute_statistics(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.normalize:
            ...
            return np.zeros((1, self.data.shape[1], 1, 1)), np.ones((1, self.data.shape[1], 1, 1))
    ...
    def train_joint_stage(self):
        ...
        for epoch in range(joint_config['epochs']):
            ...
            if epoch % 5 == 0 or epoch == joint_config['epochs'] - 1:
                ...
                if val_metrics['loss'] < self.best_metrics.get('joint_val_loss', float('inf')):
...
class SpatiotemporalDataManager:
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            self.train_loader = data_module.train_dataloader(
                ...
                worker_init_fn=lambda worker_id: seed_worker_fn(worker_id, self.config.experiment.get('seed', 42))
            )
    ...
    def _create_dummy_loaders(self) -> None:
        ...
        # 创建虚拟数据
        dummy_data = torch.randn(16, self.config.data.T_in, self.config.data.channels, 
                                 self.config.data.img_size, self.config.data.img_size)
        dummy_target = torch.randn(16, self.config.data.T_out, self.config.data.channels,
                                  self.config.data.img_size, self.config.data.img_size)
...
class RealDataARTrainer:
    ...
    def __init__(self):
        ...
        output_dir = Path(self.config.experiment.get('output_dir', 'runs')) / self.config.experiment.get('name', 'default_exp')
        self.log_manager = LogManager(self.config, output_dir=output_dir, is_primary=True)

        # 初始化数据与模型/优化器管理器
        self.data_manager = DataManager(self.config)
        self.model_manager = ModelManager(self.config.get('model', {}), self.device_manager.device)
        self.optimizer_manager = OptimizerManager(self.config.get('training', {}), model=None)
        self.loss_manager = LossManager(self.config.get('loss', {}))
        self.curriculum_manager = CurriculumManager(self.config, logger=logging.getLogger('trainer'))
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            # 模型
            # 同步最新模型配置与设备
            self.model_manager.config = self.config.get('model', self.config.model if hasattr(self.config, 'model') else {})
            ...
            # 同步最新训练配置
            self.optimizer_manager.cfg = self.config.get('training', self.config.training if hasattr(self.config, 'training') else {})
...
class SpatiotemporalTrainer:
    ...
    def _setup_logger(self) -> logging.Logger:
        ...
        # 创建文件处理器
        log_file = self.output_dir / 'training.log' if hasattr(self, 'output_dir') else 'training.log'
    ...
    def _train_spatial_stage(self) -> bool:
        """训练空间阶段"""
        self.current_stage = 'spatial'
        self.logger.info(f"开始空间预训练，周期数: {self.config.training.spatial_pretrain.epochs}")
    ...
    def _train_temporal_stage(self) -> bool:
        """训练时间阶段"""
        self.current_stage = 'temporal'
        self.logger.info(f"开始时间预训练，周期数: {self.config.training.temporal_pretrain.epochs}")
    ...
    def _train_joint_stage(self) -> bool:
        """训练联合微调阶段"""
        self.current_stage = 'joint'
        self.logger.info(f"开始联合微调，周期数: {self.config.training.joint_finetune.epochs}")
    ...
    def _basic_joint_training(self) -> None:
        ...
        for epoch in range(num_epochs):
            ...
            # 记录指标
            self.logger.info(f"周期 {epoch + 1}: 训练损失={train_loss:.6f}, 验证损失={val_metrics.get('loss', 0):.6f}")
    ...
    def _train_one_epoch(self) -> float:
        ...
        for batch_idx, (inputs, targets) in enumerate(self.data_manager.train_loader):
            ...
            # 计算损失（简化版本）
            loss = nn.MSELoss()(outputs.get('final_pred', outputs.get('spatial_pred')), targets)
...
示例:
    ...
    parser.add_argument('--resume', type=str, 
                       help='恢复训练的检查点路径')
    parser.add_argument('--output-dir', type=str, default='runs/spatiotemporal',
                       help='输出目录（默认: runs/spatiotemporal）')
    parser.add_argument('--device', type=str, default='cuda',
                       help='设备类型（cuda/cpu，默认: cuda）')
    parser.add_argument('--seed', type=int, default=2025,
                       help='随机种子（默认: 2025）')
    parser.add_argument('--test-only', action='store_true',
                       help='仅进行测试，不进行训练')
    parser.add_argument('--stage', type=str, choices=['spatial', 'temporal', 'joint', 'all'],
                       default='all', help='训练阶段（默认: all）')
