...
class RealDataARTrainer:
                ...
                if dl_cfg is not None:
                    ...
                    if num_workers > 0 and prefetch_factor is not None:
                        ...
                        try:
                            ...
                            supports_pmd = False
                    # 在CPU设备上彻底禁用pin_memory_device与pin_memory，避免触发pin线程
                    if (isinstance(self.device, torch.device) and self.device.type == 'cpu') or (not torch.cuda.is_available()):
                        dl_kwargs['pin_memory'] = False
                        dl_kwargs.pop('pin_memory_device', None)
                        supports_pmd = False
                    elif supports_pmd and pin_memory:
                        try:
                            if torch.cuda.is_available():
                                # 使用明确的 cuda:{index}
                    ...
                    if dl_kwargs.get('pin_memory', False) and ('pin_memory_device' not in dl_kwargs or dl_kwargs.get('pin_memory_device') is None):
                        ...
                        try:
                            ...
                            if 'pin_memory_device' in str(e):
                                ...
                                raise
                        # 修复：仅在CUDA环境下设置pin_memory_device；CPU环境不设置，避免触发pin线程
                                ...
                                if _dl is not None and hasattr(_dl, 'pin_memory_device'):
                                    ...
                                    if _pmd is None or (isinstance(_pmd, str) and len(_pmd) == 0):
                                        if torch.cuda.is_available() and isinstance(self.device, torch.device) and self.device.type == 'cuda':
                                            dev_index = 0 if (self.device.index is None) else int(self.device.index)
                                            if getattr(self, 'distributed', False):
                                                dev_index = int(getattr(self, 'local_rank', dev_index))
                                            setattr(_dl, 'pin_memory_device', f"cuda:{dev_index}")
                                        else:
                                            # 在CPU或无CUDA时，确保不设置pin_memory_device
                                            try:
                                                delattr(_dl, 'pin_memory_device')
                                            except Exception:
                                                setattr(_dl, 'pin_memory_device', None)
                            except Exception:
                                pass
                        try:
    ...
    def create_visualizations(self, sample_batch: Optional[Dict] = None, epoch: int = 0):
        """Create visualizations (Unified ARTrainingVisualizer usage)"""
        # 简化版：若可视化器不可用则跳过
        try:
        ...
        if self.visualizer is None:
            self.logger.debug("Visualizer unavailable, skipping create_visualizations")
            return
        try:
            ...
            self.logger.debug(f"create_visualizations failed: {_viz_err}")

    def create_test_visualizations(self, final_test_metrics: Optional[Dict] = None):
        """Test-phase simple visualization placeholder"""
        if self.visualizer is None:
            self.logger.debug("Test visualization skipped: visualizer unavailable")
            return
        try:
            # 保存测试指标摘要到可视化目录
            ...
            self.logger.info("🖼️ Test-phase visualizations and metrics saved")
        except Exception as _tviz_err:
            self.logger.debug(f"create_test_visualizations failed: {_tviz_err}")
...
"""
    ...
    def train(self):
        ...
        try:
            # 采样间隔：performance_monitoring.report_interval_seconds（秒），同时兼容 interval_sec
            try:
                perf_cfg = getattr(self.config, 'performance_monitoring', None)
                if perf_cfg is not None:
            ...
            # 兼容两种实现：utils.performance.ResourceMonitor 与 utils.resource_monitor.ResourceMonitor
            import inspect
            resource_monitor_started = False
            try:
                from utils import performance as perf_mod
                PerfRM = getattr(perf_mod, 'ResourceMonitor', None)
            except Exception:
                PerfRM = None
            try:
                from utils import resource_monitor as legacy_mod
                LegacyRM = getattr(legacy_mod, 'ResourceMonitor', None)
            except Exception:
                LegacyRM = None

            if PerfRM is not None:
                # 检查构造签名，优先使用新版写 JSONL 的实现
                sig = inspect.signature(PerfRM.__init__)
                if 'output_dir' in sig.parameters:
                    resource_monitor = PerfRM(str(self.output_dir), interval_sec=mon_interval)
                    # 新版提供 start/stop
                    if hasattr(resource_monitor, 'start'):
                        resource_monitor.start()
                        resource_monitor_started = True
            if not resource_monitor_started and LegacyRM is not None:
                # 回退到旧版，仅日志记录
                legacy_sig = inspect.signature(LegacyRM.__init__)
                if 'log_interval' in legacy_sig.parameters:
                    resource_monitor = LegacyRM(log_interval=mon_interval)
                    # 旧版提供 start_monitoring/stop_monitoring
                    if hasattr(resource_monitor, 'start_monitoring'):
                        resource_monitor.start_monitoring()
                        resource_monitor_started = True

            if resource_monitor_started:
                self.logger.info("📈 资源监控已启动（自动兼容实现），输出目录: resource_metrics.jsonl/训练日志")
            else:
                self.logger.warning("资源监控未启动：未找到可用实现或启动方法")
        except Exception as e:
        ...
        try:
            ...
            if testing_enabled and run_final_test:
                ...
                if best_ckpt_path.exists():
                    ...
                    self.logger.info("ℹ️ Best checkpoint not found, skipping final test evaluation")
            else:
                self.logger.info("⏭️ testing.enabled=false or run_final_test=false, skipping final test phase")
