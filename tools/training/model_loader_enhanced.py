...
class EnhancedModelLoader:
    ...
    UTILITY_CLASSES = {
        'Up', 'Down', 'DoubleConv', 'BasicBlock', 'Bottleneck',
        'PatchEmbed', 'PatchMerging', 'Mlp', 'Attention', 'Block',
        'WindowAttention', 'DropPath', 'LayerNorm', 'GELU',
        'Conv2d', 'Linear', 'ReLU', 'BatchNorm2d', 'GroupNorm',
        'AdaptiveAvgPool2d', 'MaxPool2d', 'AvgPool2d',
        'Upsample', 'ConvTranspose2d', 'Sequential',
        'BasicLayer', 'SwinTransformerBlock', 'PatchExpanding',  # Component blocks
        'Rearrange', 'FNOBottleneck',  # Utility operations
    }
    ...
    MODEL_SPECIFIC_CONFIGS = {
        ...
        'SwinUNet': {
            ...
            'optional_params': {
                ...
                'window_size': 8, 'patch_size': 4, 'mlp_ratio': 4.0,
        ...
        'SparseSwinUNet': {
            'required_params': ['in_channels', 'out_channels', 'img_size'],
            'optional_params': {
                'embed_dim': 96, 'depths': [2, 2, 6, 2], 'sparse_ratio': 0.5
            },
            'parameter_mapping': {
                'in_channels': ['in_channels', 'in_ch', 'n_channels'],
                'out_channels': ['out_channels', 'out_ch', 'n_classes'],
        ...
        'TemporalConv1D': {
            ...
            'optional_params': {
                'hidden_channels': 64, 'num_layers': 4, 'kernel_size': 3,
                'dilation_base': 2, 'dropout': 0.1, 'activation': 'gelu'
            },
            'parameter_mapping': {
                'in_channels': ['in_channels', 'in_ch', 'n_channels']
            },
        ...
        'TemporalEncoder': {
            ...
            'optional_params': {
                'hidden_dim': 128, 'num_conv_layers': 4, 'kernel_size': 3,
                'dilation_base': 2, 'dropout': 0.1, 'use_positional_encoding': True
            },
            'parameter_mapping': {
        ...
        'TemporalTransformerEncoder': {
            ...
            'optional_params': {
                ...
                'dropout': 0.1, 'causal': True, 'max_seq_len': 64
            },
            'parameter_mapping': {
        ...
        'SwinUNetDecoder': {
            ...
            'optional_params': {
                ...
                'window_size': 8, 'skip_connections': True
            },
        ...
        'UNetDecoder': {
            'required_params': ['encoder_channels', 'decoder_channels'],
            'optional_params': {
                'skip_connections': True, 'upsampling_mode': 'bilinear'
            },
        ...
        'DecoderBlock': {
            ...
            'optional_params': {
                'skip_channels': 0, 'upsampling_mode': 'bilinear'
            },
    ...
    MODEL_SPECIFIC_CONFIGS.update({
        ...
        'FNO2d': {
            'required_params': ['in_channels', 'out_channels', 'img_size'],
            'optional_params': {
                'modes1': 12, 'modes2': 12, 'width': 64, 'n_layers': 4, 'activation': 'gelu'
            },
            'parameter_mapping': {
                'in_channels': ['in_channels', 'in_ch', 'n_channels'],
                'out_channels': ['out_channels', 'out_ch', 'n_classes'],
        ...
        'MLPMixer': {
            'required_params': ['in_channels', 'out_channels', 'img_size'],
            'optional_params': {
                'embed_dim': 512, 'mlp_ratio': (0.5, 4.0), 'patch_size': 16, 'depth': 8, 'drop_rate': 0.0, 'drop_path_rate': 0.0
            },
            'parameter_mapping': {
                'in_channels': ['in_channels', 'in_ch', 'n_channels'],
                'out_channels': ['out_channels', 'out_ch', 'n_classes'],
    ...
    def _process_module(self, module_name: str):
        ...
        try:
            ...
            if module is None:
                logger.warning(f"All import strategies failed for {module_name}: {import_errors}")
                return
                ...
                if issubclass(obj, nn.Module) and obj != nn.Module:
                    if name in self.UTILITY_CLASSES:
                        ...
                        if self._is_valid_model_class(obj):
                            ...
                            self.utility_registry[name] = obj
                            logger.debug(f"Registered as utility (not valid model): {name}")
    ...
    def _is_valid_model_class(self, cls: Type[nn.Module]) -> bool:
        ...
        try:
            ...
            has_standard_params = any(
                ...
                ['in_channels', 'in_ch', 'in_chans', 'n_channels', 'input_channels',
                 'out_channels', 'out_ch', 'out_chans', 'n_classes', 'output_channels',
                 'img_size', 'image_size', 'input_size', 'd_model', 'input_dim']
            )
    ...
    def _map_parameters_with_config(self, input_params: Dict[str, Any], model_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for required_param in model_config.get('required_params', []):
            ...
            if not found and required_param in model_config.get('optional_params', {}):
                mapped_params[required_param] = model_config['optional_params'][required_param]
        
        # Add optional parameters that are present in input
        for optional_param, default_value in model_config.get('optional_params', {}).items():
    ...
    def _convert_parameter_types_advanced(self, params: Dict[str, Any], target_class: Type[nn.Module], model_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for param_name, param_info in sig.parameters.items():
                if param_name != 'self':
                    param_types[param_name] = self._extract_type_from_annotation(param_info.annotation)
            
            # Convert each parameter based on expected type
            for param_name, param_value in params.items():
                if param_name in param_types and param_types[param_name] is not None:
                    converted_value = self._convert_value_to_type(param_value, param_types[param_name])
            ...
            logger.warning(f"Advanced type conversion failed for {target_class.__name__}: {e}")
            return params
    
    def _extract_type_from_annotation(self, annotation: Any) -> Optional[Type]:
    ...
    def _handle_special_input_types(self, model: nn.Module, model_config: Dict[str, Any], test_input: Optional[torch.Tensor] = None) -> torch.Tensor:
    ...
    def create_model(self, model_name: str, config: Optional[Dict[str, Any]] = None, **kwargs) -> nn.Module:
        """
        ...
        converted_params = self._convert_parameter_types_advanced(mapped_params, model_class, model_config)
        ...
        try:
            ...
            if has_kwargs:
                ...
                if missing_required:
                    inferred_params = self._infer_missing_parameters(missing_required, model_config, converted_params)
                    valid_params.update(inferred_params)
                    
                    # Check again for missing parameters
                    still_missing = [p for p in missing_required if p not in valid_params]
                    if still_missing:
    ...
    def _infer_missing_parameters(self, missing_params: List[str], model_config: Dict[str, Any], available_params: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # Special inference rules
                ...
                if 'img_size' in available_params:
                    ...
                    if isinstance(img_size, (int, float)):
                        inferred[missing_param] = (img_size // 4, img_size // 4)  # Typical downsampling
                ...
                if 'in_channels' in available_params:
                    ...
                    inferred[missing_param] = [in_channels * 8, in_channels * 4, in_channels * 2, in_channels]
                ...
                if 'out_channels' in available_params:
                    ...
                    inferred[missing_param] = [out_channels * 8, out_channels * 4, out_channels * 2, out_channels]
                ...
                if 'embed_dim' in available_params:
                    ...
                    inferred[missing_param] = available_params['in_channels'] * 32  # Typical expansion
                ...
                if 'num_heads' in available_params:
                    inferred[missing_param] = available_params['num_heads'][0] if isinstance(available_params['num_heads'], list) else available_params['num_heads']
                else:
                    inferred[missing_param] = 8  # Default head count
        
        return inferred
    
    def test_model_forward(self, model: nn.Module, input_shape: Optional[Tuple] = None) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Model {getattr(model, '_model_name', type(model).__name__)} forward test successful: {test_input.shape} → {output.shape}")
            return True
            
        except Exception as e:
            logger.warning(f"Model forward test failed for {getattr(model, '_model_name', type(model).__name__)}: {e}")
            return False
    ...
    def list_models(self, category: Optional[str] = None) -> List[str]:
        ...
        for model_name in self.model_registry.keys():
            ...
            if category == 'complete' and config.get('input_type', 'image') == 'image' and 'required_params' in config:
                if 'in_channels' in config['required_params'] and 'out_channels' in config['required_params']:
                    filtered_models.append(model_name)
            elif category == 'temporal' and config.get('input_type') == 'sequence':
                filtered_models.append(model_name)
            elif category == 'decoder' and 'encoder_channels' in config.get('required_params', []):
                filtered_models.append(model_name)
            elif category == 'attention' and 'nhead' in config.get('required_params', []):
                filtered_models.append(model_name)
        ...
        for registered_name, model_class in self.model_registry.items():
            if registered_name.lower().replace('_', '') == normalized_name:
                try:
                    ...
                    sig = inspect.signature(model_class.__init__)
                    params = {name: str(param) for name, param in sig.parameters.items() if name != 'self'}
                    ...
                    return {
                        ...
                        'docstring': model_class.__doc__ or "No documentation available"
                    }
                except Exception as e:
                    return {
                        'name': registered_name,
    ...
    def _categorize_model(self, config: Dict[str, Any]) -> str:
        ...
        if config.get('abstract', False):
            ...
            return 'attention'
        elif 'in_channels' in config.get('required_params', []) and 'out_channels' in config.get('required_params', []):
            return 'complete'
        else:
            return 'other'
...
def create_enhanced_model(model_name: str, config: Optional[Dict[str, Any]] = None, **kwargs) -> nn.Module:
    """
...
def list_enhanced_models(category: Optional[str] = None) -> List[str]:
...
def test_enhanced_model(model_name: str, config: Optional[Dict[str, Any]] = None, **kwargs) -> bool:
...
if __name__ == "__main__":
    ...
    for model_name in test_models:
        try:
            ...
            print(f"✓ {model_name}: Created and tested successfully ({'✓' if success else '✗'})")
        except Exception as e:
