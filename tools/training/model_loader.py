...
class ModelLoader:
                ...
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    if (issubclass(obj, nn.Module) and 
                        ...
                        try:
                            ...
                            # 检查是否有必要的初始化参数
                            if any(param in params for param in ["in_channels", "in_ch", "input_channels"]):
    ...
    def create_model(
        ...
        **kwargs
        ...
        logger.info(f"使用内部模型类: {model_info['class_name']} from {model_info['file_path']}")
        ...
        if config is not None:
            try:
                ...
                # 也支持顶层配置
                for key in ["in_channels", "out_channels", "img_size", "input_channels", "output_channels"]:
        ...
        for standard_name, aliases in param_mapping.items():
            for alias in aliases:
                if alias in final_params:
                    ...
                    logger.debug(f"参数映射: {alias} -> {standard_name} = {final_params[alias]}")
                    break
                ...
                try:
                    ...
                    if "in_channels" in missing_params:
                        if "channels" in data_config:
                            ...
                            logger.info(f"从data配置推断 in_channels: {data_config['channels']}")
                        elif "input_channels" in data_config:
                            normalized_params["in_channels"] = data_config["input_channels"]
                            logger.info(f"从data配置推断 in_channels: {data_config['input_channels']}")
                    
                    if "out_channels" in missing_params:
                        if "channels" in data_config:
                            normalized_params["out_channels"] = data_config["channels"]
                            logger.info(f"从data配置推断 out_channels: {data_config['channels']}")
                        elif "output_channels" in data_config:
                            normalized_params["out_channels"] = data_config["output_channels"]
                            logger.info(f"从data配置推断 out_channels: {data_config['output_channels']}")
                    
                    if "img_size" in missing_params:
                        if "img_size" in data_config:
                            normalized_params["img_size"] = data_config["img_size"]
                            logger.info(f"从data配置推断 img_size: {data_config['img_size']}")
                        elif "image_size" in data_config:
                            normalized_params["img_size"] = data_config["image_size"]
                            logger.info(f"从data配置推断 img_size: {data_config['image_size']}")
        ...
        for param_name, converter in type_conversions.items():
            if param_name in normalized_params:
                try:
                    ...
                    if param_name == 'mlp_ratio':
                        if isinstance(original_value, (list, tuple)):
                            ...
                            try:
                                ...
                                converted_value = parsed if isinstance(parsed, (list, tuple)) else float(parsed)
                            except Exception:
                    ...
                    logger.debug(f"参数类型转换 {param_name}: {original_value} -> {converted_value}")
        ...
        for param_name in list_params:
            if param_name in normalized_params and isinstance(normalized_params[param_name], str):
                try:
                    # 尝试解析类似 "[2, 2, 6, 2]" 的字符串
                    import ast
                    original_value = normalized_params[param_name]
                    ...
                    normalized_params[param_name] = converted_value
                    logger.debug(f"列表参数转换 {param_name}: {original_value} -> {converted_value}")
                except Exception as e:
                    logger.warning(f"无法解析列表参数 {param_name}: {normalized_params[param_name]}, 错误: {e}")
        ...
        try:
            ...
            for param_name, param_value in normalized_params.items():
                if param_name in valid_params:
                    ...
                    logger.warning(f"参数 {param_name} 不在模型 {model_class.__name__} 的构造函数中，将被忽略")
            ...
            if required_params:
                ...
                for param_name in required_params:
                    if param_name == 'in_ch' and 'in_channels' in filtered_params:
                        ...
                        logger.info(f"映射参数 in_channels -> in_ch")
                    elif param_name == 'out_ch' and 'out_channels' in filtered_params:
                        filtered_params['out_ch'] = filtered_params['out_channels']
                        logger.info(f"映射参数 out_channels -> out_ch")
                    elif param_name == 'in_channel' and 'in_channels' in filtered_params:
                        filtered_params['in_channel'] = filtered_params['in_channels']
                        logger.info(f"映射参数 in_channels -> in_channel")
                    elif param_name == 'out_channel' and 'out_channels' in filtered_params:
                        filtered_params['out_channel'] = filtered_params['out_channels']
                        logger.info(f"映射参数 out_channels -> out_channel")
                    elif param_name == 'input_channels' and 'in_channels' in filtered_params:
                        filtered_params['input_channels'] = filtered_params['in_channels']
                        logger.info(f"映射参数 in_channels -> input_channels")
                    elif param_name == 'output_channels' and 'out_channels' in filtered_params:
                        filtered_params['output_channels'] = filtered_params['out_channels']
                        logger.info(f"映射参数 out_channels -> output_channels")
                    elif param_name == 'image_size' and 'img_size' in filtered_params:
                        filtered_params['image_size'] = filtered_params['img_size']
                        logger.info(f"映射参数 img_size -> image_size")
                    elif param_name == 'input_size' and 'img_size' in filtered_params:
                        filtered_params['input_size'] = filtered_params['img_size']
                        logger.info(f"映射参数 img_size -> input_size")
            ...
            if missing_required:
                error_msg = f"模型 {model_class.__name__} 缺少必需参数: {missing_required}"
                logger.error(error_msg)
                raise ValueError(error_msg)
    ...
    def _validate_model_attributes(self, model: nn.Module, params: Dict[str, Any]) -> None:
        """
        验证模型基本属性
        ...
        try:
            ...
            if not isinstance(model, nn.Module):
                raise ValueError(f"创建的模型不是 nn.Module 实例，实际类型: {type(model)}")
            
            # 验证输入输出通道数
            if hasattr(model, 'in_channels') and 'in_channels' in params:
                model_in_channels = getattr(model, 'in_channels')
                expected_in_channels = params['in_channels']
                if model_in_channels != expected_in_channels:
                    logger.warning(f"模型输入通道数不一致: 期望={expected_in_channels}, 实际={model_in_channels}")
            
            if hasattr(model, 'out_channels') and 'out_channels' in params:
                model_out_channels = getattr(model, 'out_channels')
                expected_out_channels = params['out_channels']
                if model_out_channels != expected_out_channels:
                    logger.warning(f"模型输出通道数不一致: 期望={expected_out_channels}, 实际={model_out_channels}")
            
            if hasattr(model, 'img_size') and 'img_size' in params:
                model_img_size = getattr(model, 'img_size')
                expected_img_size = params['img_size']
                if model_img_size != expected_img_size:
                    logger.warning(f"模型图像尺寸不一致: 期望={expected_img_size}, 实际={model_img_size}")
    ...
    def validate_model_compatibility(self, model: nn.Module, config: Dict[str, Any]) -> bool:
        """
        验证模型兼容性
        ...
        try:
            # 检查输入输出通道
            expected_in_channels = config.get("in_channels", config.get("data", {}).get("channels", 1))
            expected_out_channels = config.get("out_channels", config.get("data", {}).get("channels", 1))
            expected_img_size = config.get("img_size", config.get("data", {}).get("img_size", 128))
            ...
            if model_in_channels is not None and model_in_channels != expected_in_channels:
                logger.warning(f"输入通道不匹配: 模型={model_in_channels}, 期望={expected_in_channels}")
                return False
                
            if model_out_channels is not None and model_out_channels != expected_out_channels:
                logger.warning(f"输出通道不匹配: 模型={model_out_channels}, 期望={expected_out_channels}")
                return False
                
            if model_img_size is not None and model_img_size != expected_img_size:
                logger.warning(f"图像尺寸不匹配: 模型={model_img_size}, 期望={expected_img_size}")
                return False
...
def check_model_health(model: nn.Module, input_shape: tuple = None) -> Dict[str, Any]:
    ...
    try:
        ...
        health_report["trainable_parameters"] = sum(p.numel() for p in model.parameters() if p.requires_grad)
        ...
        try:
            ...
            with torch.no_grad():
                ...
                health_report["output_shape"] = list(test_output.shape) if hasattr(test_output, 'shape') else str(type(test_output))
        ...
        health_report["status"] = "healthy" if not health_report["errors"] else "unhealthy"
...
def get_model_info(model_name: str, models_dir: str = None) -> Optional[Dict[str, Any]]:
    """获取模型信息"""
    loader = get_model_loader(models_dir)
