...
y = H(u) + n.
$$
其中，$H$ 代表**观测口径**，用于刻画从真实场到观测数据的确定性生成流程；噪声项 $n$ 作为加性扰动独立注入。$H$ 不仅定义抽样位置与掩码，更应显式覆盖工程实现中的关键细节，例如：
...
为避免符号混用，本文用 $[0,T]$ 表示连续时间区间，用 $T_s$ 表示离散时间步数（time steps）。在离散化实现中，真实场与观测数据通常表示为张量：
$$
u\in\mathbb{R}^{B\times T_s\times C\times H\times W}.
$$
观测数据 $y$ 可表示为：
- 规则网格观测：$y\in\mathbb{R}^{B\times T_s\times C\times h\times w}$；
- 点采样观测：索引集合 $\mathcal{I}\subset\{1,\dots,H\}\times\{1,\dots,W\}$；
- 缺失/遮挡：掩码 $m\in\{0,1\}^{B\times T_s\times 1\times H\times W}$（或其下采样版本）。
...
- **超分辨（SR）口径**：在缩小阶段采用抗混叠策略（先低通滤波、再缩小），并使用面积型重采样等方法实现降采样；工程实现可将 `INTER_AREA` 作为参考实现，并显式固化边界策略与对齐规则；
...
### 1.4.4 时空耦合与长时预测的累积误差

时空物理场通常表现出复杂的时空耦合特性。在稀疏观测下，直接进行端到端的时空联合建模往往面临优化困难，容易陷入局部最优。此外，在自回归式的长时预测任务中，微小的初始重建误差或边界伪影会随着时间步的推进而指数级放大，导致长时推断发散。如何设计有效的训练策略以解耦空间重建与时序演化，并控制累积误差，是另一大挑战。
...
神经算子方法以“函数到函数”的映射视角学习参数化解算子，支持跨输入函数、跨参数族与跨离散网格的推断。FNO 与 DeepONet 是两类代表性结构：前者利用 Fourier 空间的全局表示以建模长程相互作用，后者利用 Branch/Trunk 分解实现算子逼近。与本文主题直接相关的是：跨分辨率/跨网格推断会遭遇离散化差异与混叠风险，统一口径与**抗混叠一致性**的设计将显著影响模型的泛化能力与审计结果。
...
1.  **提出“统一观测算子（Unified Observation Operator）”方法论框架**：
    本文将观测算子 $H$ 确立为数据生成与模型评测的唯一逻辑入口，并强制训练端的退化算子 $DC$ 在实现细节（插值核、抗混叠预滤、边界策略、对齐规则）上与 $H$ 保持严格镜像。这一设计从根本上消除了隐性域偏差，确保了实验结论在真实观测口径下的可复现性与工程可落地性。
