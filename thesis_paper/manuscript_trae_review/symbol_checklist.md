...
- **fRMSE (Frequency RMSE)**
  $$
  \mathrm{fRMSE}(\mathcal{K}) = \sqrt{\frac{1}{|\mathcal{K}|}\sum_{k\in\mathcal{K}} \left| |\tilde{U}_k| - |U_k| \right|^2}
  $$
  其中 $\mathcal{K}$ 分为 low/mid/high 三段（详见第6章定义），$\|\cdot\|$ 表示取模（关注能量谱误差）。

- **DC Error (Data Consistency Error)**
  同 $H_{\mathrm{err}}$，即 $\|H(\tilde{u})-y\|_2$。
