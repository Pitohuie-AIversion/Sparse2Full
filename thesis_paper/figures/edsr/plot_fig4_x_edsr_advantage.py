...
# Output directory changed to match requirements
OUTPUT_DIR = "."
...
def main():
    ...
    gs = fig.add_gridspec(2, 2, width_ratios=[1, 1], height_ratios=[1, 1], wspace=0.3, hspace=0.4)
    ...
    try:
        ...
        if os.path.exists(img_path) and os.path.exists(unet_path):
            ...
            ax_a.axis('off')
            ax_a.set_title("(a) Reconstruction (GT | UNet | EDSR)", fontsize=10, loc='center', y=-0.15)
            ...
            ax_c.axis('off')
            ax_c.set_title("(c) Detail Amplification (GT | UNet | EDSR)", fontsize=10, loc='center', y=-0.15)
        else:
    ...
    ax_b.set_xscale('log')
    ax_b.set_title("(b) Pareto Frontier (PSNR vs Params)", fontsize=10, loc='center', y=-0.25)
    ...
    ax_d.set_title("(d) Reconstruction Error Comparison", fontsize=10, loc='center', y=-0.3)
    
    ax_d.text(3, 0.0025, "* p<0.05", ha='center', va='bottom', fontweight='bold', color='red', fontsize=10)
    
    # Save using bbox_inches=tight to avoid warning
