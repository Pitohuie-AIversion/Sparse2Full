...
本章的目标不是“把欠定问题变成严格可解”，而是给出一条可证明、可审计、可复现的理论链条，解释为何本文提出的三类约束能够把学习过程稳定地引导到**评测口径一致且结构合理**的解族（如图 3-1 所示的完整框架）。
...
- **Q3：为何“序列化时空训练”能比端到端训练更有效地收敛？（见 4.5.5 课程学习视角）**
...
实际工程中的观测算子 $H$ 常由“高斯预滤 + 插值缩放 + 边界策略”等模块组合而成。OpenCV 明确给出 `resize` 在缩小图像时推荐 `INTER_AREA`，并提供 `GaussianBlur` 的参数语义（核大小、$\sigma_{\mathrm{blur}}$、`borderType` 等），这些都使 $H$ 的实现细节可被明确固化并复用。
...
$$
H(u)=D_s(G_{\sigma_{\mathrm{blur}}} * u),
$$

其中 $G_{\sigma_{\mathrm{blur}}}$ 为高斯低通核，$D_s$ 为缩小采样。OpenCV 对缩小采样推荐 `INTER_AREA`，并对 `GaussianBlur` 给出可核验参数语义。
...
### 4.5.5 序列化训练的课程学习视角 (Theoretical Motivation for Sequential Training)

时空耦合模型的端到端训练（End-to-End Training）常面临优化地形崎岖、收敛速度慢且易陷入局部极小值的问题。本文提出的“空间预训练 $\to$ 时序预训练 $\to$ 联合微调”策略，其理论依据可从**课程学习（Curriculum Learning）**与**分解学习（Decomposition Learning）**的角度进行解释：

1.  **降低任务复杂度（Complexity Reduction）**：
    时空重建任务可分解为两个正交子任务：$T_{spatial}$（从稀疏观测恢复全场）与 $T_{temporal}$（学习流体动力学演化）。
    - 若直接联合训练，模型需同时寻找满足 $y_t = H(u_t)$ 与 $u_{t+1} = \mathcal{N}(u_t)$$ 的解，解空间搜索难度呈指数级上升。
    - 序列化训练首先在 $T_{spatial}$ 上找到较优的初始参数点 $\theta_{spatial}^*$，再在 $T_{temporal}$ 上找到 $\theta_{temporal}^*$，最后在联合空间微调。这本质上利用了先验知识将非凸优化问题转化为一系列更简单的凸（或良态）子问题。

2.  **特征质量保证（Feature Quality Assurance）**：
    在时序模块（如 LSTM/Transformer）训练初期，若输入的空间特征（来自未训练的空间编码器）充满噪声，时序模块将难以捕捉真实的动力学规律，甚至过拟合噪声。空间预训练确保了进入时序模块的 latent feature 具备明确的物理语义，从而加速时序模块的收敛。

3.  **缓解 Exposure Bias**：
    Teacher Forcing Decay 策略使得模型在训练初期依赖真实历史（Ground Truth），逐步过渡到依赖自身预测。这在理论上减小了训练分布（Conditioned on GT）与测试分布（Conditioned on Prediction）之间的差异（Distribution Shift），增强了长时预测的鲁棒性。
...
为了验证命题 4.1，我们在第 6 章设计了专门的“口径一致性消融实验”（见 6.3.2 节）。

- **正例（Consistent）**：$DC\equiv H$（同实现、同 $\sigma_{\mathrm{blur}}=1.0$）。实验显示，在此设置下，EDSR 和 UNet 均达到了最高的 PSNR（64.75 dB / 48.95 dB）和最低的 $H_{\mathrm{err}}$。
- **负例（Mismatch）**：故意错配退化参数（$\sigma_{\mathrm{blur}}^{\mathrm{train}}=2.0 \neq \sigma_{\mathrm{blur}}^{\mathrm{val}}=1.0$）。实验结果表明，尽管 Rel-L2 变化不大，但 $H_{\mathrm{err}}$ 激增 30% 以上，且 PSNR 显著下降（-0.8 ~ -1.1 dB），证实了“口径错配导致理论上界失效”的推论。
