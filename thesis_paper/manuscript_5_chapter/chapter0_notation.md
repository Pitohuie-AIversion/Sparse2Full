...
| $u^{(z)}$ | 张量 | 真实场的 z-score 标准化表示，用于模型训练 |
| $y$ | 张量 | 稀疏观测数据，由真实场 $u$ 经观测算子作用并叠加噪声后得到 |
| $\tilde{u}$ | 张量 | 模型重建的预测场 (原值域)，与 $u$ 具有相同的分辨率和维度，用于最终评测 |
| $\hat{u}^{(z)}$ | 张量 | 神经网络直接输出的预测场 (z-score 域)，经反归一化后得到 $\tilde{u}$ |
...
| **算子与映射** | | |
| $H(\cdot)$ | 算子 | **观测算子 (Observation Operator)**。定义了从高分辨率真实场到稀疏观测的映射，包含降采样、裁剪、掩码及抗混叠滤波等物理过程。输入必须为原值域变量 (如 $u$ 或 $\tilde{u}$)。 |
...
| $f_\theta(\cdot)$ | 函数 | 深度神经网络重建模型，参数为 $\theta$，输入为观测 $y$ 及辅助信息，输出为 $\hat{u}^{(z)}$ |
| $\mathcal{F}(\cdot)$ | 变换 | 傅里叶变换 (Fourier Transform)，用于频域分析和谱损失计算 |
| **损失函数** | | |
| $\mathcal{L}_{total}$ | 标量 | 总损失函数，用于模型反向传播优化 |
| $\mathcal{L}_{rec}$ | 标量 | **重建损失 (Reconstruction Loss)**。通常为 $L_1$ 或 $L_2$ 范数，衡量 $\hat{u}^{(z)}$ 与 $u^{(z)}$ 的逐点逼近程度 |
| $\mathcal{L}_{spec}$ | 标量 | **谱一致性损失 (Spectral Consistency Loss)**。衡量 $\hat{u}^{(z)}$ 与 $u^{(z)}$ 在频域（特别是低频段）的幅度/相位一致性 |
| $\mathcal{L}_{dc}$ | 标量 | **观测一致性损失 (Observation Consistency Loss)**。衡量 $H(\tilde{u})$ 与 $y$ 的差异，即 $H_{\mathrm{err}}$ 的平方形式 |
| $\lambda_{\mathrm{spec}}, \lambda_{\mathrm{dc}}$ | 标量 | 损失函数中的加权超参数，用于平衡各项贡献 |
| **评价指标** | | |
| $\text{Rel-}L_2$ | 标量 | 相对 $L_2$ 误差，$\frac{\| \tilde{u} - u \|_2}{\| u \|_2}$，核心评价指标 |
| $H_{\mathrm{err}}$ | 标量 | **评测口径误差 (Evaluation Consistency Error)**，$\| H(\tilde{u}) - y \|_2$，衡量预测结果是否符合观测约束 |
