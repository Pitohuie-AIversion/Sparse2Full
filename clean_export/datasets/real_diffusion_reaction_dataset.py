...
class RealDiffusionReactionDataset(Dataset):
    ...
    def __init__(
        ...
        window_cache_max_gb: Optional[float] = None,
        ...
        self._h5: Optional[h5py.File] = None  # worker-local handle, lazily opened
        ...
        self._window_cache_max_bytes: int = int((window_cache_max_gb or 0) * (1024**3)) if window_cache_max_gb else 0
    ...
    def _ensure_h5_open(self) -> h5py.File:
        ...
        if self._h5 is None:
            if self._memory_map_dataset:
                ...
                try:
                    self._h5 = h5py.File(self.h5_file_path, 'r', driver='core', backing_store=False)
    ...
    def preload_all_samples(self, limit: Optional[int] = None):
        """可选：将全部样本的data预加载到RAM以提速"""
        keys = self.sample_keys[:limit] if (limit is not None and limit > 0) else self.sample_keys
    ...
    def _window_cache_get(self, key: Tuple[str, int, int]) -> Optional[torch.Tensor]:
    ...
    def _window_cache_put(self, key: Tuple[str, int, int], tensor: torch.Tensor):
        ...
        # 控制容量与字节上限
        while True:
            over_capacity = (self._window_cache_capacity > 0 and len(self._window_cache) > self._window_cache_capacity)
            over_bytes = (self._window_cache_max_bytes > 0 and self._window_cache_bytes > self._window_cache_max_bytes)
    ...
    def _analyze_data_structure(self):
        ...
        if len(data_shape) == 4:
            ...
            print(f"   样本数: {self.n_samples} (样本ID: {self.sample_keys[0]}-{self.sample_keys[-1]})")
    ...
    def _generate_sequence_indices(self):
        ...
        for sample_idx in range(self.n_samples):
            ...
            if start_max < start_min:
                continue
            for start_time in range(start_min, start_max + 1, max(1, self.time_step_stride)):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if cached is not None:
            ...
            try:
                window_np = sample_data[t_start:t_start + t_total]  # [T_total, H, W, C]
            except Exception:
                # 兼容极端情况：逐步读取回退
                arrs = []
            ...
            if isinstance(self.channel_index, int):
                ...
                idx = torch.tensor([int(i) for i in self.selected_channels], dtype=torch.long)
...
class RealDiffusionReactionDataModule(pl.LightningDataModule):
    ...
    def __init__(self, config: DictConfig):
        ...
        # RAM预加载与缓存开关
        self.preload_entire_dataset = dl_cfg.get('preload_entire_dataset', False) or getattr(getattr(config, 'hardware', DictConfig({})), 'memory', DictConfig({})).get('preload_data', False)
        ...
        self.memory_map_dataset = bool(dl_cfg.get('memory_map_dataset', False) or getattr(getattr(config, 'hardware', DictConfig({})), 'memory', DictConfig({})).get('memory_map_dataset', False))
        from collections import OrderedDict
        self._batch_cache: "OrderedDict[Any, Dict[str, torch.Tensor]]" = OrderedDict()
        self._batch_cache_bytes: int = 0
        self._batch_cache_max_bytes: int = int((self.batch_cache_max_gb or 0) * (1024**3)) if self.batch_cache_max_gb else 0
        ...
        self.prefill_cache_batch_size = int(dl_cfg.get('prefill_cache_batch_size', self.batch_size))

        # 若启用整数据RAM预加载：仅当使用非fork上下文时强制单进程，fork可共享页面
        if self.preload_entire_dataset:
            use_fork = False
            try:
                if self.mp_context is not None and hasattr(self.mp_context, 'get_start_method'):
                ...
                use_fork = False
            if not use_fork:
                print("⚠️ preload_entire_dataset=True 且非fork 上下文，已将 num_workers 强制为 0 以避免多进程复制RAM缓存")
                self.num_workers = 0
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            ...
            try:
                if self.batch_cache_enabled and self.prefill_cache_on_setup and self.prefill_cache_batches > 0:
                    import random as _rnd
                    total_prefill = int(self.prefill_cache_batches)
                    bsz = int(self.prefill_cache_batch_size)
                    print(f"🧺 预填充批次缓存: batches={total_prefill}, batch_size={bsz}")
                    for b in range(total_prefill):
                        # 从数据集长度随机采样索引（注意：__getitem__ 的 idx 是 self.indices 的位置）
                        sel = [_rnd.randrange(len(self.train_dataset)) for _ in range(bsz)]
                        ...
                        if (b + 1) % max(1, (total_prefill // 10)) == 0:
                            ...
                            print(f"   进度: {b+1}/{total_prefill}, 当前批次缓存占用≈{gb:.1f} GB")
                    gb = self._batch_cache_bytes / (1024**3)
    ...
    def _batch_cache_get(self, key: Tuple[Tuple[str, int], ...]) -> Optional[Dict[str, torch.Tensor]]:
    ...
    def _batch_cache_put(self, key: Tuple[Tuple[str, int], ...], batch: Dict[str, torch.Tensor]):
        if not self.batch_cache_enabled:
            return
        ...
        while True:
            over_capacity = (self.cache_lru_size > 0 and len(self._batch_cache) > self.cache_lru_size)
            over_bytes = (self._batch_cache_max_bytes > 0 and self._batch_cache_bytes > self._batch_cache_max_bytes)
            if not (over_capacity or over_bytes):
                break
            k, v = self._batch_cache.popitem(last=False)
            self._batch_cache_bytes -= self._estimate_batch_bytes(v)

    def _collate_fn(self, samples: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        batch = {
            ...
            'sample_idx': torch.tensor([s['sample_idx'] for s in samples], dtype=torch.long),
            'start_time': torch.tensor([s['start_time'] for s in samples], dtype=torch.long),
