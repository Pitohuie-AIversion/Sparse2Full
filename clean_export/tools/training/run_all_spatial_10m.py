#!/usr/bin/env python3
"""
Script to run training for classified spatial models with ~10M parameters on Diffusion-Reaction 2D dataset.
Uses single GPU execution (CUDA:1) and specific configuration.
...
MODEL_CATEGORIES = {
    "Target": ["edsr", "nafnet", "UformerLite", "uno", "ConvUNetLite", "UNet", "stablefno2d", "RestormerLite"]
}
...
def main():
    ...
    # We enforce 'runs_drd_paper' as the root for this experiment suite
    ...
    for category, models in MODEL_CATEGORIES.items():
        ...
        for model_name in models:
            ...
            # Format: AR-DR2D-{model_name}-SRx4-10M-300ep
            exp_prefix = f"AR-DR2D-{model_name}-SRx4-10M-300ep"
            ...
            # Construct command - Use standard python for single GPU to avoid DDP overhead
            cmd = [
                sys.executable,
            ...
            # Prepare environment with CUDA_VISIBLE_DEVICES=1
            env = os.environ.copy()
            env["CUDA_VISIBLE_DEVICES"] = "1"

            print(f"Command: {' '.join(cmd)} (on CUDA:1)")
