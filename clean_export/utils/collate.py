...
def fast_collate_fn(batch: List[Optional[Dict[str, Any]]]) -> Optional[Dict[str, Any]]:
    """
    快速专用的collate函数：
    - 过滤None项
    ...
    try:
        ...
        sample_idx = torch.tensor([int(it.get('sample_idx', -1)) for it in items], dtype=torch.int64)
        start_time = torch.tensor([int(it.get('start_time', 0)) for it in items], dtype=torch.int64)
