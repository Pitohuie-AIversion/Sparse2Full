...
PDEBench稀疏观测重建系统数据集扩展方案，旨在充分利用E:/2D目录下的所有PDE数据类型，实现多物理场景的稀疏观测重建能力。该方案支持达西流、扩散反应、不可压缩Navier-Stokes等多种偏微分方程的数据处理和模型训练。
...
| 页面名称 | 模块名称 | 功能描述 |
|---------|----------|----------|
| 数据集管理页面 | 数据扫描模块 | 自动扫描E:/2D目录，识别所有HDF5文件类型和结构 |
...
1. **数据发现阶段**：系统扫描E:/2D目录，自动识别所有PDE数据类型
