...
insert_text = """**分析**：仅在无噪数据上训练的模型对高频噪声较敏感。此外，在 **2D Darcy Flow** 数据集上的验证表明，“统一观测算子 + 残差骨干”的范式具备处理不同物理机制的通用潜力。
...
old_text = """**分析**：仅在无噪数据上训练的模型对高频噪声较敏感。此外，在 **2D Darcy Flow** 数据集上的验证表明，“统一观测算子 + 残差骨干”的范式具备处理不同物理机制的通用潜力。"""

for file_path in files:
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Renumber tables first
    content = content.replace('表 4-12', '表 4-13')
    content = content.replace('表 4-11', '表 4-12')

    # Replace the text
    content = content.replace(old_text, insert_text)
