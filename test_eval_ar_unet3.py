...
def evaluate_rollout(ckpt_path, config_path, t_in):
    ...
    with torch.no_grad():
        for i, batch in enumerate(trainer.test_loader):
            ...
            try:
                # Use standard test pipeline logic
                model = trainer.get_model()
                
                # Manual AR loop for basic UNet
                pred_seq = []
                curr_in = input_seq[:, -1:] # Take last frame (B, 1, C, H, W)
                for t in range(T):
                    # For ARWrapper or direct model that expects (x, steps)
                    if hasattr(model, 'autoregressive_predict'):
                        # But wait, autoregressive_predict usually takes (input_seq, steps) and does the loop internally
                        pass
                        
                    # Let's just do a manual loop. UNet expects (B, C, H, W)
                    x = curr_in[:, 0] # (B, C, H, W)
