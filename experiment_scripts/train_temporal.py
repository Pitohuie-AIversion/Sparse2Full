...
class TemporalTrainer:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        def _metric_to_float(v: Any) -> float:
            if isinstance(v, torch.Tensor):
                if v.numel() > 1:
                    return float(v.mean().detach().cpu().item())
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            with torch.no_grad():
                ...
                elif pred_for_metrics.ndim == 5 and target_seq.ndim == 4:
                    pred_for_metrics = pred_for_metrics[:, -1]
                ...
                if pred_for_metrics.shape[-2:] != target_for_metrics.shape[-2:]:
                    ...
                    if pred_for_metrics.ndim == 5:
                        B, T, C = pred_for_metrics.shape[:3]
                        pred_bt = pred_for_metrics.reshape(B * T, C, *pred_for_metrics.shape[-2:])
                        pred_bt = F.interpolate(
                            pred_bt,
                            size=target_size,
                            mode="bilinear",
                            align_corners=False,
                        )
                        pred_for_metrics = pred_bt.reshape(B, T, C, *target_size)
                    else:
                        pred_for_metrics = F.interpolate(
                            pred_for_metrics,
                            size=target_size,
                            mode="bilinear",
                            align_corners=False,
                ...
                rel_l2 = _metric_to_float(metrics.get("rel_l2", 0.0))
                mae = _metric_to_float(metrics.get("mae", 0.0))
    ...
    def evaluate(self, loader: DataLoader, desc: str = "Evaluating") -> Dict[str, float]:
        self.model.eval()
        losses = []
        metrics_acc: Dict[str, List[float]] = {"rel_l2": [], "mae": [], "psnr": [], "ssim": []}
            ...
            for batch in tqdm(loader, desc=desc):
                ...
                for k in metrics_acc:
                    if k in m:
                        ...
                        if isinstance(v, torch.Tensor):
                            v = (
                                v.mean().detach().cpu().item()
                                if v.numel() > 1
                                else v.detach().cpu().item()
                            )
                        metrics_acc[k].append(float(v))

        out: Dict[str, float] = {"loss": float(np.mean(losses))} if losses else {"loss": 0.0}
