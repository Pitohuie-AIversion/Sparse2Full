...
try:
    ...
    print("Error: Could not import tools.generate_paper_package. Ensure you are running from the project root or minimal_export directory.")
    sys.exit(1)

def main():
    parser = argparse.ArgumentParser(description="Generate a reproducible paper package from a training run.")
    parser.add_argument("--run-dir", type=str, required=True, help="Path to the training run directory (e.g., runs/exp_name)")
    parser.add_argument("--output-dir", type=str, default="paper_package", help="Directory to save the package (default: paper_package)")
    parser.add_argument("--include-checkpoints", action="store_true", help="Include model checkpoints (may be large)")
