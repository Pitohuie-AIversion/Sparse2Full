...
def validate_temporal_inputs(
    ...
    if teacher is not None:
        if teacher.dim() != 5:
            raise ValueError(f"Teacher sequence must be 5D [B,T,C,H,W], got {teacher.dim()}D")
        
        if teacher.size(0) != x_seq.size(0):
            raise ValueError(f"Batch size mismatch: input {x_seq.size(0)} vs teacher {teacher.size(0)}")
...
def create_temporal_model_wrapper(model: nn.Module, **kwargs) -> nn.Module:
    ...
    # 为模型添加时间序列预测方法
    def temporal_predict(self, x_seq, T_out, teacher=None, train_mode=True, **pred_kwargs):
