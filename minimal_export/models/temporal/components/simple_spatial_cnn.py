...
class SimpleSpatialTemporalCNN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() == 5:  # [B, T, C, H, W]
            ...
            temporal_features = spatial_features.permute(0, 2, 1, 3, 4)  # [B, hidden_channels, T, H, W]
            for conv in self.temporal_conv:
                temporal_features = self.activation(conv(temporal_features))
            
            # Take last timestep or average
            temporal_features = temporal_features.permute(0, 2, 1, 3, 4)  # [B, T, hidden_channels, H, W]
...
if __name__ == "__main__":
    ...
    model = SimpleSpatialCNN(in_channels=3, out_channels=1, hidden_channels=32).to(device)
    ...
    model_temporal = SimpleSpatialTemporalCNN(in_channels=3, out_channels=1, hidden_channels=32).to(device)
