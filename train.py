...
class Trainer:
    ...
    def _verify_data_consistency(self) -> None:
        ...
        try:
            ...
            observation = sample_batch.get('original_observation', sample_batch.get('lr_observation', sample_batch['observation']))
    ...
    def _build_model_input(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        - 支持 `use_lowres_input`：直接提取未经插值的低分辨率物理场 (如 32x32)
        - 否则默认处理时序 baseline（取最后一个时间步或添加 batch 维）并追加 `coords` 与 `mask` 通道
        - 依据模型的 `in_channels` 对最终通道进行裁剪/填充对齐
        """
        # 检查是否启用直接低维输入 (兼容 DataParallel module)
        model_unwrapped = getattr(self.model, 'module', self.model)
        use_lowres = getattr(model_unwrapped, 'use_lowres_input', False)
        if not use_lowres and hasattr(self.config, 'model') and hasattr(self.config.model, 'params'):
            use_lowres = getattr(self.config.model.params, 'use_lowres_input', False)
        ...
        # 获取模型实际期望的总输入通道数 (兼容 DataParallel)
        ...
        if expected_in is not None:
            ...
            if model_input.shape[1] != expected_in:
                allow_ch_mismatch = getattr(getattr(self.config, 'training', None), 'allow_channel_mismatch', True)
                ...
                if model_input.shape[1] > expected_in:
                    self.logger.warning(f"model_input channels ({model_input.shape[1]}) trimmed to expected {expected_in}")
                    model_input = model_input[:, :expected_in]
                else:
                    pad_ch = expected_in - model_input.shape[1]
                    self.logger.warning(f"model_input channels ({model_input.shape[1]}) padded with {pad_ch} zeros to reach expected {expected_in}")
                    pad = torch.zeros(
    ...
    def _prepare_target(self, target: torch.Tensor, pred_shape: Tuple[int, int, int, int]) -> torch.Tensor:
        ...
        if target.shape[1] != pred_shape[1]:
            allow_ch_mismatch = getattr(getattr(self.config, 'training', None), 'allow_channel_mismatch', False)
            ...
            self.logger.warning(f"target channels ({target.shape[1]}) trimmed to match prediction channels ({pred_shape[1]})")
            target = target[:, :pred_shape[1]]

        # 空间尺寸对齐与物理完整性校验
        if target.shape[-2:] != pred_shape[-2:]:
            allow_resample = getattr(getattr(self.config, 'training', None), 'allow_target_resampling', False)
            ...
            self.logger.warning(f"Target physical field resized from {target.shape[-2:]} to {pred_shape[-2:]} via bilinear interpolation.")
