        ...
        for i, batch in enumerate(trainer.test_loader):
            ...
            try:
                ...
                for t in range(T):
                    ...
                    # Pure basic manual AR
                    if hasattr(model, 'autoregressive_predict'):
                        pred = model.autoregressive_predict(curr_in, 1, teacher=None, train_mode=False)
                        pred = pred[:, -1]
                    else:
                        try:
                            if hasattr(model, 'spatial_forward'):
                                pred = model.spatial_forward(x).spatial_pred
                            else:
                                pred = model(x)
                        except:
                            try:
